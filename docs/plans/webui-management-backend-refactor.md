...
1. 一个客户端只有一个 `client_id` 唯一标识，名称仅供显示。保留 ID 优先、IP/CIDR 回退的客户端策略匹配；IP 匹配不能补造请求原始 ID。
...
本方案同时给出配置编辑、查询和实时推送的后端接入步骤，但不扩大系统配置的可写范围：`logs` 可写，`database`、`webui`、`work` 仍只读。设计通过不等于授权执行其中的数据迁移或删除操作。

## 2. 现状与差异

| 核对对象 | 当前源码事实 | 目标变化 |
| --- | --- | --- |
| [客户端模型](../../backend/src/config/model.rs)、[匹配索引](../../backend/src/policy/client.rs) | 以资源名称定位，`match.ids` 为数组，ID 匹配后可回退 CIDR；IP 匹配参与策略和缓存身份 | 单个 ID 为唯一键；名称可重复；保留 ID/IP 策略匹配与最长前缀规则 |
...
| `name` | 可选显示名称，可修改、可重复；不是引用键或统计维度 |
...
客户端 ID 创建后在普通编辑界面只读。变更 ID 表示新身份，不做历史记录重命名；如后续需要专门的 ID 迁移操作，应另行定义。DNS 上游改名仍遵守上位需求中的旧名称定位约束，不能套用客户端 ID 的只读交互。

IP/CIDR 是匹配条件，不是客户端的第二个主键。保留当前最长前缀规则：不同前缀的包含关系允许，重复的规范化 CIDR 或同等优先级冲突在保存/prepare 时拒绝，不能靠列表顺序选择。名称重复不构成匹配歧义。IPv4-mapped IPv6 的归一化规则在 transport、配置和匹配端保持一致。
...
旧数据可能缺失事实，因此允许 `null`，但必须用独立的 `identity_status` 区分“本次确实未传 ID”和“旧版本未记录”；匹配来源缺失使用 `legacy_unknown`，不冒充 `none`。不能按现有配置推导历史原始 ID、匹配结果或客户端名称。
...
建议默认每 5 分钟生成快照，可配置正 duration；没有内存变化时可跳过。过程如下：
...
需要验证 Windows/Linux 的同目录替换、权限和中断恢复；不能把两文件操作描述为跨文件原子事务。快照间隔到了但上一轮未完成时合并任务，不并发写同一个文件。
...
`records_path` 与统计库路径属于只读系统配置，可通过部署配置迁移设置，不开放普通 WebUI 修改路径。日文件的文件名必须由已解析日期生成，不能直接拼接来自请求的字符串。
...
| 能力 | 后端工作 |
| --- | --- |
| 客户端资源 | 单 ID 定位、名称/IP/CIDR 修改、最长前缀及冲突校验；移除旧 `match.ids` 数组，保留 `match.ips` |
| 查询列表/详情 | 原始身份、当时匹配 ID/来源、当前名称、目录 revision、历史缺失状态和跨日游标；保留有界 Answer 与缓存生产上游信息 |
...
继续沿用 Management 的登录、CSRF/Origin 校验、受限错误和只读字段保护。所有写操作携带预期配置 revision，冲突时拒绝覆盖；上游改名以旧名称定位并在一次候选校验中处理引用关系。配置提交后返回“已保存”“已应用”“需要重启”三个可区分状态，不把写文件成功当成运行时应用成功。

客户端显示名称修改不要求重启；IP/CIDR 匹配规则、客户端覆盖和内存预算走候选校验后原子应用；快照周期/保留参数在下一任务边界读取新 revision；日志和系统绑定/路径仍遵守既有重启限制。首次用户写入事务不是现成的全模块配置事务，需逐类扩展并测试失败恢复，不能直接扩大反序列化后的整份配置写回权限。
...
这是配置、存储和 API 的协同破坏性变化，建议发布新配置 schema 和管理 API 版本。版本值及正式请求字段在实施提交中统一落地，不在当前版本偷偷复用旧字段的新含义。

| 旧内容 | 迁移处理 |
| --- | --- |
| 单个 `match.ids` | 可预览迁为该实际 ID，旧名称成为显示名称，保留原 IP/CIDR 和覆盖行为 |
| 多个 `match.ids` | 预览拆分为多个 ID；用户指定哪一项保留 IP/CIDR 条件，不能复制成冲突规则；其他覆盖可按确认结果复制 |
| 只有 IP/CIDR 的客户端 | 由用户分配配置唯一 ID，保留 IP/CIDR 策略匹配；实际无 ID 请求的 `client_id` 仍为空，不能将分配的 ID 回填到请求字段 |
| 缓存 SQLite | 不强制导入；新路径冷启动并生成新快照，旧缓存仅作可删除的遗留文件，不重命名伪装格式 |
| 持久化容量字段 | 新 schema 明确移除并报告，不把它迁成统计大小参考 |
| 详情条数阈值/硬上限 | 明确废弃；不能从条数推算容量或天数，预览新的 R/G/T 后确认 |
| 原 `max_record_age` | 可作为建议 R 的来源，非整天需显式说明取整提案；G/T 不自动猜测 |
| 旧详情 `client_bucket` | 不可还原原始客户端 ID 或匹配方式，不迁入原始 ID；默认标记旧匹配未知，仅在有可靠历史版本映射时另行明确转换匹配 ID，禁止按当前同名配置猜测 |
| 旧统计客户端名称维度 | 保留为带版本的 legacy 维度，查询不与新 ID 维度混算；到期后按同一策略自然淘汰 |

业务迁移须在明确授权的维护窗口中执行：备份原业务库及 WAL 一致状态，停止旧 writer，按事件 UTC 日流式复制详情至新分片，验证记录数/日期范围/摘要，更新 layout 版本和 manifest，最后切换新 reader/writer。迁移复制阶段不按新期限擅自丢弃历史记录；新策略首次清理也必须遵守已确认的配置。

旧详情表退出服务前不物理清理，之后的删除/空间整理另作显式维护步骤；不得为了“迁移完成”自动 VACUUM 或删除备份。旧详情中无法恢复的 ID 永远保持缺失；即使同名配置仍存在，也不能据此补写。

回退依赖旧程序、旧配置和切换前一致备份，新程序写入后的数据不能保证被旧程序识别。先停写再回退，说明新旧期间可能的数据差距；不能对已清理历史承诺可恢复。缓存回退允许直接冷启动，不把可丢失缓存提升成业务迁移依赖。

## 9. 实施顺序

| 阶段 | 主要修改边界 | 验收门槛 |
| --- | --- | --- |
| B1 契约与迁移预览 | config model/resolve/validate/migrate、ports、API schema 草案 | 客户端唯一键与 R/G/T 语义确定；旧配置歧义显式报告 |
...
| B5 详情日分片 | storage detail writer/read model、布局迁移、跨日游标 | 批写无 COUNT/历史清理；跨日排序、迟到写入和连接上限正确 |
...
| B8 集成与文档收口 | 配置模板、受影响架构/实现文档、WebUI 联调 | 新旧迁移、故障注入、高频负载及可回退发布验收 |

各阶段不以大规模改名或同时替换全部 adapter 开始；先建立 typed 契约和测试，再逐段替换生产装配。未通过迁移验收不得删除旧路径的回退证据。
...
2. 客户端改名、同名、CIDR 包含与冲突、删除/ID 复用、后来新增 ID；历史匹配不重算，未知实际 ID 可按现有规则回退 IP 但原始值不变。
3. IP 规则修改影响后续策略与正确缓存隔离，不改已完成请求的匹配结果或统计归属；名称不改变缓存，旧 `client_bucket` 不补成实际 ID。
...
13. 真实 SQLite 升级与维护回退演练；旧 ID 无法恢复时保持标记，迁移不自动删历史。
14. 在受控高频压测中证明批写路径没有历史 COUNT/DELETE/VACUUM；报告吞吐、延迟分位、内存峰值、快照 I/O 峰值与凌晨清理耗时，不用静态检查代替性能结果。

计划退出条件为：上述新逻辑已在生产装配接线、API/前端对齐、迁移和关键故障验收完成；实现事实沉淀至对应 `docs/implementation/`，被改变的契约同步到 `docs/architecture/`，再按文档维护规则删除本方案及索引项。本轮仅完成方案和图稿调整，以上测试均为后续执行要求。
...
[本地评审总览](webui-management-designs/review.html)保留其余模块。图稿是静态演示，不代替类型校验、真实查询、保存交互、迁移或运行验收。
