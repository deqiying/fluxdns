...
2026-09-07 的[确认决策](../webui-management-decisions.md)覆盖旧图中的三类示意：客户端 name 改为必填唯一，日志改为热应用，解析分页采用前后 cursor；旧版 legacy 数据兼容也已取消。本轮重订计划不重画 SVG，实施必须按修订文字替换相关标注。外部文件提示/还原/修改及未同步状态见[配置专项](../webui-management-config-runtime-plan.md)，作为图外必要交互验收。
...
- 客户端 ID 唯一、IP/CIDR 继续参与策略匹配；旧图同名示意已被上述决定替代。
...
| 系统配置 | [系统概览](webui-system-settings.svg)：日志可编辑，其余只读 | [日志编辑](webui-logs-editor.svg)：保留控件，重启提示改为应用/同步状态 |
...
图中使用演示数据，按钮不执行操作。原图稿阶段的结构/视觉检查不代表本次重订计划重新渲染验收；本轮只检查文档一致性和链接，真实交互与浏览器验证由前端计划执行。
