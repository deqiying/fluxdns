...
| 计划 | 文档状态 | 计划状态 | 剩余范围 |
| --- | --- | --- | --- |
| [后端契约验证开发计划](backend-contract-validation.md) | 有效 | 实施中 | 本机定向矩阵、三处契约修复及 1,024 会话驱动已交付；V3 剩余网络实测/分类门槛、V5–V10 环境/恢复/长期负载仍未整体验收 |
| [WebUI 管理后台重构需求](webui-management-requirements.md) | 草案 | 待评审 | 确认 12 个一级模块、配置编辑、实时推送需求与 28 张视觉草案（含深色服务状态、解析详情和配置编辑）；后续开展详细设计和前后端实施 |
