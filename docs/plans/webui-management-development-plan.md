...
这些是详细计划的评审输入，不是实施者可以自行忽略的 TODO。采纳建议时，把正式值落到 schema、源码契约及对应文档，不保留两套相互竞争的定义。

| 编号 | 决策主题 | 本计划建议及必须明确的内容 | 阻塞任务 |
| --- | --- | --- | --- |
| D-01 | 版本与切换 | 新配置 schema 和新 Management API major；建议采用下一主版本，确认正式版本值、schema 文件名、生成入口及认证路由迁移。同版前后端一起切换，不要求双写或旧 WebUI 兼容 | BE-01、FE-01 |
| D-02 | 配置提交 | 全局源配置 revision 与运行 revision 分离；校验、持久化、应用各有结果。明确“已保存但未应用”、重试和日志待重启时后续热配置提交的处理 | BE-02、FE-02 |
| D-03 | 资源改名与操作范围 | 上游及组按旧名称定位，在同一候选中按类型更新全部引用；其他可改名资源采用同样的受限引用遍历。客户端普通编辑不改 ID。不增加顶层删除能力 | BE-08、FE-05 至 FE-10 |
| D-04 | 指标口径 | 按有效 DNS 请求接入计数；最近 60 秒请求数 / 60 为平均 QPS，最近 600 秒请求数 / 10 为平均 RPM。在线身份按原始 ID、缺 ID 时有效 IP 去重；缺数、窗口未满、基数超限必须显式表示 | BE-09、FE-03 |
| D-05 | 推送与分页 | HTTP 返回有界快照和 opaque cursor；WS 使用提交顺序游标而非事件时间补齐，记录 ID 去重。保留倒序、页大小和前后翻页，不承诺日分片下任意页码直跳；图中数字分页需据此确认调整 | BE-05、BE-07、BE-10、FE-04 |
| D-06 | 保留默认值与限制 | 评审原方案建议的 R=7、G=3、T=1 GiB、快照 5 分钟；明确自定义天数合法上界、查询分段/超时、时区来源、容量精度及删除影响确认 | BE-01、BE-06、FE-07 |
| D-07 | 配置可见信息 | 允许已认证管理者读取批准字段的源配置路径和 SecretRef 来源，用于对应表单；仍不暴露解析后的 Secret、密码 hash、全量配置或运行时派生绝对路径。须更新现有 Management 更严格的只读投影约束 | BE-07、BE-08、FE-10、FE-11 |
| D-08 | 依赖与平台 | 在既有 Axum 上评估 WS feature；分别选择进程信息采样、服务器时区与图表实现方案，记录复用现有能力和新增依赖的取舍。新依赖/工具按项目规则审批，不能直接假定已经安装 | BE-06、BE-09、BE-10、FE-03 |
| D-09 | 图稿外状态 | 以浅色 12 页为基准，服务状态深色样例纳入验证，其余深色页不默认宣称完成。补齐窄屏、隐藏类型分支、键盘和表单错误态；图中“所有服务正常”只能来自真实状态 | FE-01、FE-02、FE-12 |
| D-10 | 迁移与回退 | 冻结旧客户端歧义映射、legacy 统计维度规则、备份恢复方式、维护窗口、首次清理确认和新写入期间回退损失告知 | BE-11、联合验收 |

D-01 至 D-07 应在依赖它们的正式接口实现前签定；D-08 至 D-10 可以并行准备，但相应能力不得带着未决项进入最终验收。选择发生实质变化时先更新本表及受影响任务。
...
- [ ] D-01 至 D-10 已形成明确决定，schema 与任务依赖一致。
...
用户本轮要求的是在方案中设计提交点，不代表批准现在提交，也不等于后续整次实施都获准自动提交。后续实施开工时应确认是逐次批准，还是明确授权按本表阶段性提交；commit 授权仍不包含 push。
...
| --- | --- | --- | --- |
| GC-01 | 三份开发计划、原方案导航、索引及经确认的计划决策 | 计划评审；V-D；仅经批准后实际提交 | `docs(webui): 制定前后端重构与阶段提交计划` |
