...
> 计划状态：待评审
>
> 代码基线：`21fd23f3f711e2f7acc712b9ff715915c5248180`（2026-09-07 用户决策修订，定向核对配置/Runtime；未运行产品验收）
...
详细顺序、任务和提交点见[总计划](webui-management-development-plan.md)、[前端](webui-management-frontend-development-plan.md)和[后端](webui-management-backend-development-plan.md)。D-01 至 D-12 已确认，不因本轮实现拆分继续重复询问，也不表示已实现。
...
- 当前开发阶段无运行服务，不考虑旧配置/数据/逻辑兼容。采用新版本直接初始化，不做旧库搬迁。
- 配置先运行时应用再写文件，外部改文件只提示处理；提供还原/修改并采用，普通保存以运行态对应源配置覆写。
- 全部已认证用户按管理员处理，无角色管理；Windows 验收、最终运行 Linux，缓存命中主链路目标 2ms 内（不含客户端 I/O）。
...
| 规则集 | 查看和编辑规则集列表及其配置。 |
| 客户端配置 | name 是唯一管理键，单 client_id 保留为请求匹配身份；IP/CIDR 匹配与覆盖继续支持。 |
...
- 各模块按旧 name 定位，新 name 在对应命名空间唯一；一次候选自动更新全部类型引用，不能按新名误建资源或留悬空引用。
- 请求详情采用“原始 ID/IP＋最小匹配结果”，分别保存实际 ID、请求 IP、匹配客户端 ID 和 ID/IP/无匹配来源，不保存客户端名称或完整配置。UDP/TCP 未传入 ID 时原始字段始终为空，IP 策略匹配结果另存。
- 缓存改为内存池周期非 SQLite 快照，失败可冷启动；统计/详情独立存储。统一保留和新格式恢复见[后端方案](webui-management-backend-refactor.md)，不再要求旧数据迁移。
- 允许破坏性设计和旧代码退出，不代表本轮执行本地数据删除。未来验收只操作明确的开发/临时目录。
- 本文不规定 API 路径、请求字段、WebSocket 消息格式、前端技术选型、组件结构或配置写入实现。

## 6. 后续设计与验收方向

指标、分页/缓冲、默认保留值、名称引用和执行边界按已确认决策落实；剩余类型字段、保护预算和热更新失败状态在对应任务内核定。重订后的计划不作为已有实现描述。
...
8. 各模块 name 唯一，客户端 ID 另作匹配身份且不可普通编辑；IP 匹配不回填原始 ID，历史不随配置变化重算，无旧版 legacy 兼容支线。
9. 缓存快照与业务历史独立；不出现缓存持久化配额或详情记录数上限。统计与详情使用同一份保留策略，详情关闭不停止统计或既有历史清理。
10. 正常编辑热应用后落盘；外部文件修改不改变 DNS，支持还原/组合采用及覆盖确认；日志不再要求每次重启。
...
客户端列显示当时匹配 ID/来源、当前 name 和请求 IP；保留当前名称语义及长文本查看。图中旧版未记录示意不再实现，未匹配和无原始 ID 仍区分；筛选不能按当前 IP 重算历史。
...
详情只展示后端实际保留的有界 Answer，不代表完整 wire。截断、空 Answer、缺失耗时分别显示原因，不构造缺失内容；旧版 `legacy_redacted` 兼容分支取消。缓存上游标为生产来源，不暗示本次再次调用。加载/失败/超长及触屏交互按前端计划验证。
...
[客户端列表](webui-management-designs/webui-clients.svg)保留 ID/name 两行及 IP/CIDR、策略和覆盖列；图中同名/可选名称示意已由 D-03 替代，实施改为 name 必填唯一。[客户端编辑](webui-management-designs/webui-client-editor.svg)的 ID 仍只读，名称可改，保存保留旧 name；其他匹配/覆盖控件继续保留。

客户端 ID 为单个必填唯一值，name 为独立唯一管理键，IP 列表可空。精确 ID 优先，其余回退最长 IP/CIDR；同规范 CIDR 冲突拒绝，不同长度可包含。IP 匹配按实际请求 IP 隔离缓存，不把整个网段合为一池；图外覆盖表单由前端计划补齐。
...
[系统配置概览](webui-management-designs/webui-system-settings.svg)仅日志可编辑，其余只读。[日志编辑](webui-management-designs/webui-logs-editor.svg)保留开关/级别/路径布局，旧“需要重启”标注失效，实施改为热应用和文件同步反馈。
