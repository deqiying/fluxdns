...
以下三张 SVG 使用同一套风格：浅灰侧栏、白色工作区、蓝色主操作、克制的状态色，以及统一的列表和表单样式。当前用于评审整体视觉方向，不是三个不同主题，也不是最终页面设计。

图中指标、地址、分组和配置均为演示数据；按钮、筛选和表单为静态示意，不代表交互或接口已实现。

### 7.1 服务状态

展示一级导航、关键指标、流量趋势、监听入口摘要与实时推送状态。

![服务状态视觉草案](webui-service-status.svg)

### 7.2 DNS 上游

展示上游与上游组的浏览入口、搜索筛选、配置列表和行内编辑入口。

![DNS 上游视觉草案](webui-dns-upstreams.svg)

### 7.3 DoH 上游编辑

展示列表之上的定制化二级弹窗，包括连接参数、代理出口和 ECS 配置。

![DoH 上游编辑视觉草案](webui-doh-upstream-editor.svg)
