...
日志先保存为待重启后，再编辑可热应用资源是必测场景。建议候选预检分离“已保存源配置”和“本进程可应用投影”，保留旧进程级字段并应用本次允许的热变更；必须复用 Runtime 的 restart-required 分类，不能把需要重启的字段偷偷生效。若该分离无法保证一致性，按 D-02 明确拒绝及恢复路径，不允许实现期间默默改成所有保存都成功。
...
以下数值为 D-04/D-08 的评审建议，确认后纳入正式契约：

| 指标 | 定义 | 边界 |
| --- | --- | --- |
| 平均 QPS | 最近 60 秒进入正常 DNS 请求处理的有效请求数 / 60 | 包含缓存、本地回答、上游失败等已接纳请求；不按详情落库量计数 |
| 平均 RPM | 最近 600 秒同口径请求数 / 10 分钟 | 与 QPS 单位/窗口不同，不直接拿当前 QPS 乘 60 |
| 趋势 | 最近 10 分钟；QPS 用 1 秒桶，RPM 用完整分钟桶分别携带时间区间 | 有共同时间轴但不同采样间隔；不补线伪造缺失点 |
| 在线客户端 | 最近 60 秒内有请求的原始 ID 去重；无 ID 则按有效 IP | 两类 key 域分隔；同名或匹配同 CIDR 不合并为一个设备 |
| 内存 | 当前 FluxDNS 进程常驻内存字节数 | 两页面共用采样，不能用系统总内存或 Rust heap 代替 |
| 线程/时长 | 当前进程线程数、启动时间、单调运行时长 | 不支持采样时返回不可用，不返回正常零值 |
...
2. 定义服务状态和解析记录订阅。建议状态每 1 秒更新、记录每 1 秒合批推送，均为代码级有界默认值，不擅自新增 YAML 调优字段。
...
5. 建立有界共享 replay buffer。建议以 60 秒、最多 5000 条和 8 MiB 三个上限中先到者为准，单连接队列另行有界；这些是评审起点，BE-12 负载前冻结。
