...
本计划按确认决策重订 B1-B8，保留身份矩阵、快照协议和保留算法，取消旧版迁移与兼容。2026-09-07 在 P0 交付后追加授权 P1 公共底座与阶段性本地提交；不 push、不自动进入 P2，用户已确认的产品方向不重新提问。
...
8. 首用户初始化继续满足“持久化用户成功后发布认证”的安全要求，但写入仲裁和活动源必须与普通配置协调；本期不新增用户管理 API。
