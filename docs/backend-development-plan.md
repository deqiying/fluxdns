...
> 状态：MVP v0.1 已完成；当前执行至阶段 86（接入 DoH forwarded header 首轮客户端地址恢复），后续重点是按需补齐 v1 的 TLS/PROXY、安全故障复现和完整观测链路。
>
> 更新日期：2026-09-02
>
...
- MVP 后置项：完整跨 Runtime 配置候选发布、DoH 入站 TLS/PROXY、SQLite busy/disk-full recovery、完整 group member/资源详情元数据、final tracing subscriber、最终故障/压力/conformance 验收。

> 注：阶段 86 后的当前数值以本节汇总表和“当前验证记录”为准；MVP 已完成不等于 v1 全部验收完成，后置项只在需要时推进。

阶段 86 已接入 DoH `forwarded_header` 首轮客户端地址恢复：仅接受可信 peer 的已配置 header，支持 `X-Forwarded-For`、`X-Real-IP`、`Forwarded` 和缺失/非法回退策略；未实现的入站 TLS/PROXY、真实 OS 故障、全量详情元数据和 final subscriber 仍属 v1 后置项。阶段 85 已接入 selected upstream 首轮观测元数据：上游配置 ID 进入 stats 有界维度，resolve detail 仅记录存在性标记；生产请求和 MVP 范围不变。阶段 84 已修复 Rustls loopback 握手测试预算：两个 live HTTPS 用例在 10s 测试 deadline 下分别通过；生产 adapter timeout 未改变。

| 口径 | 当前值 | 说明 |
| --- | ---: | --- |
| 模块方案覆盖率 | 100% | 本计划覆盖 12 个后端顶层模块，每个模块均有独立方案文档 |
| 后端代码实现进度 | **71.6%** | 主要请求、Runtime、策略、资源、缓存、Storage、client bucket/selected upstream 观测元数据、DoH forwarded header 和 telemetry writer/真实输出/级别过滤首轮链路已接入；当前剩余工作集中在跨 Runtime 候选发布、DoH 入站 TLS/PROXY、SQLite busy/disk-full 故障注入与恢复验收、完整 group member/资源详情元数据、typed final subscriber/监督任务接线及最终故障验收。各模块的实现细节和证据见对应模块文档。 |
| v1 交付总进度 | **74.4%** | 设计阶段 10% 已完成，加上实现与验收部分的 `90% × 71.6%` |
...
| Transport | `backend/src/transport/*` | [transport.md](backend-modules/transport.md) | 已完成 | 实现中 | 56% | 11% |
...
```text
4% × 55% + 8% × 35% + 10% × 100% + 12% × 65% + 11% × 56% + 10% × 62% + 8% × 70% + 10% × 99% + 9% × 66% + 7% × 90% + 8% × 84% + 3% × 65% ≈ 71.6%
```
...
跨模块验收失败时，相关模块不能标记 100%。例如 DoH 请求可以返回结果但 cancellation、TLS/PROXY trust boundary 或 DNS/HTTP 错误分层未验证时，Transport 仍不得标记“已验证”。
...
当前后端代码实现进度为 71.6%，v1 交付进度为 74.4%，MVP v0.1 已完成。Config、Runtime、Transport、Upstream、Policy、Resource 和 Cache 的首轮链路已建立；Storage 已完成 SQLite adapter、detail writer、StatsPersistenceWorker、统一生命周期 facade、首轮服务生产接线、pending 内存保护边界、首轮 degraded/recovery 状态转换及 policy source/cache/strategy/client bucket/selected upstream 首轮元数据落库；DoH 已接入 forwarded header 首轮客户端地址恢复；Observability 已完成稳定 telemetry ports 的有界 writer/flush 边界。后续只按产品需要推进 v1 后置项，不为 MVP 追加完整 metadata 或安全扩展。
...
当前边界：HTTP/1.x 仍按读取顺序处理，已实现 `forwarded_header` 的可信 peer 校验、链解析和 missing/invalid 策略；未实现入站 TLS terminate/external 握手、PROXY v1/v2、HTTP/2 和完整资源/故障注入验收；上游 DoH HTTPS/TLS adapter 已完成 live handshake 验证。
