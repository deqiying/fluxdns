...
当前阶段 5 边界：Reqwest/Rustls live TLS handshake 已通过 loopback server 和验证根证书完成；parallel group late-result sink 仍未实现。阶段 6 已接入当前 PolicyDnsCore snapshot-local optimistic refresh 与有界 `LateCacheFinalizer`，但 Runtime 级 finalizer shutdown owner、最新 Runtime snapshot 捕获和完整 resource/service snapshot 生命周期仍未实现。
