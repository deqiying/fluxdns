...
| Upstream | `backend/src/upstream/*` | [upstream.md](backend-modules/upstream.md) | 已完成 | 实现中 | 98% | 10% |
...
```text
4% × 45% + 8% × 35% + 10% × 100% + 12% × 35% + 11% × 50% + 10% × 50% + 8% × 55% + 10% × 98% + 9% × 50% + 7% × 65% + 8% × 35% + 3% × 30% ≈ 56.3%
```
...
当前阶段 5 边界：proxy HTTPS、连接池 key/复用、TLS live handshake 验证、nested group、parallel late window、late cache finalizer、Cache 接线和 Runtime live resource/service snapshot 接线仍未实现。
