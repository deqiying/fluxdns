...
当前阶段 5 边界：TLS live handshake 验证、late cache finalizer 消费接线、Cache/Core 接线和 Runtime live resource/service snapshot 接线仍未实现。
...
第七个小阶段（已完成）：`PolicyDnsCore` 为 upstream 请求构造稳定 cache key，接入 fresh lookup、miss single-flight、leader admission/CAS 写回和 follower 共享结果；缓存 store 失败时降级到上游，hosts[] 本地命中仍绕过 response cache。新增配置启用缓存后的 upstream 命中测试；optimistic refresh、Moka/SQLite persistence 和 Runtime live snapshot 接线仍未实现。
