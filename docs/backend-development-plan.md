...
| Upstream | `backend/src/upstream/*` | [upstream.md](backend-modules/upstream.md) | 已完成 | 实现中 | 76% | 10% |
...
```text
4% × 45% + 8% × 35% + 10% × 100% + 12% × 35% + 11% × 50% + 10% × 50% + 8% × 55% + 10% × 76% + 9% × 50% + 7% × 65% + 8% × 35% + 3% × 30% ≈ 54.1%
```
...
当前阶段 5 边界：自定义 transport 的 bootstrap resolver 注入、HTTPS/TLS DoH adapter、SOCKS5/SOCKS5H 握手/认证/socket dial、真实 outbound 执行、真实 fallback/重试链路、Cache 接线和 Runtime snapshot 接线仍未实现。
