...
> 状态：MVP v0.1 已完成；当前已完成至阶段 120（配置驱动的 ECS 上游 query 与 cache key）。后续优先补齐配置驱动的正常运行主线、协议组合和最终验收；暂不把服务器重启/宕机恢复或缓存、请求记录的绝对持久化作为阻塞项。
>
> 更新日期：2026-09-03
>
...
| 后端代码实现进度 | **76.9%** | 以模块代码和验证证据计算，不因文档完成虚增 |
| v1 交付总进度 | **79.2%** | `10% × 设计完成度 + 90% × 后端代码实现进度` |
...
```text
4%×60% + 8%×40% + 10%×100% + 12%×71% + 11%×72%
+ 10%×67% + 8%×74% + 10%×99% + 9%×73% + 7%×90%
+ 8%×84% + 3%×90% ≈ 76.9%
```
...
| 10 | 进行中 | 资源刷新、配置 reload、安全边界和最终验收持续补齐 | 当前最新小阶段为 120 |
...
| 115–120 | 运行中资源 live publish、cache 开关契约、Application service reload、进程级配置分类、client-visible TTL/ECS | service 32 项；本阶段 message 12 项、Policy plan 8 项、Policy Core 27 项通过 |

### 当前阶段验证

```text
rustfmt --edition 2024 backend/src/dns/message.rs backend/src/policy/plan.rs backend/src/dns/policy.rs backend/src/upstream/registry.rs
cargo test --manifest-path backend/Cargo.toml dns::message::tests
cargo test --manifest-path backend/Cargo.toml policy::plan::tests
cargo test --manifest-path backend/Cargo.toml dns::policy::tests
cargo test --manifest-path backend/Cargo.toml upstream::registry::tests
git diff --check
```

以上均通过；阶段 120 未重复全量 `cargo fmt/test`。各小阶段的详细命令和输出保留在对应提交，模块级证据保留在 `docs/backend-modules/*.md`。
