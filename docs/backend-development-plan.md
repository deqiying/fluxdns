...
| Runtime | `backend/src/runtime/*` | [runtime.md](backend-modules/runtime.md) | 已完成 | 实现中 | 40% | 12% |
...
| Policy | `backend/src/policy/*` | [policy.md](backend-modules/policy.md) | 已完成 | 实现中 | 60% | 8% |
...
| Resource | `backend/src/resource/*` | [resource.md](backend-modules/resource.md) | 已完成 | 实现中 | 70% | 7% |
...
```text
4% × 45% + 8% × 35% + 10% × 100% + 12% × 40% + 11% × 50% + 10% × 55% + 8% × 60% + 10% × 99% + 9% × 50% + 7% × 70% + 8% × 35% + 3% × 30% ≈ 56.6%
```
...
第五个小阶段（已完成）：实现 const/file hosts 与 rule-set loader、稳定文件 fingerprint、UTF-8/大小/symlink/稳定读取边界，以及 `ResourceSnapshot`/registry 的版本 CAS 发布；资源 loader、snapshot 定向测试通过。remote fetch、manifest/content 原子落盘与恢复校验已补齐，scheduler/coordinator 的 Runtime-facing 纯逻辑编排也已完成，一次性 remote fetch/parse/persist worker 已接入 refresh reservation，真实 Runtime supervisor task 仍未接入。
