...
- [x] 完成 nested group sink 传播和 RuntimeCoordinator finalizer owner/current-target；旧 Runtime owner 淘汰时清理无活动 finalizer，`PolicyLateResultSink` 已按当前 `CacheEntry.quality` 使用 `CacheCondition::Version` 允许更优 late Positive 替换早期 Negative，并拒绝同级 Negative/Positive 或更低 Failure 覆盖；
- [x] 完成代理、TLS、算法和并发测试。

阶段证据：最近一次大阶段全量 `cargo test --manifest-path backend/Cargo.toml --locked` 为 539 passed、0 failed；reqwest 使用 `rustls-no-provider` 并由项目统一安装 `ring` provider，两个 loopback live TLS handshake 及并行全量测试均通过；Registry 12 项覆盖显式 direct upstream ECS 构造；Policy Core 定向测试验证 direct/group member `custom` ECS 已进入真实 DoH wire、member 覆盖 global、strategy 覆盖 member且成员 ECS group 不误用缓存；upstream executor 14 项通过，覆盖 selector、fallback、timeout、parallel fast-terminal、late-window preference、nested sink 传播和 hosts/DoH cross-adapter 并发矩阵；outcome 和 group 测试覆盖 terminal/retryable/cancelled 聚合、配置顺序与成员 ID；已有测试继续覆盖 direct connect_ip、bootstrap resolver、HTTP envelope、cancellation、SOCKS5/SOCKS5H handshake、proxy DoH loopback、pool 复用、Host/SNI、真实 TLS、响应边界、late cache 质量替换与 Runtime finalizer owner shutdown。

阶段 176 使用真实 `HostsExchange` 与通过 DoH envelope/ID 校验的 `DohExchange` 组成 parallel group，验证早期 Positive 的成员归属与 late NoData 候选均被保留；`upstream::executor::tests` 14 项通过。
