...
`DnsService::with_default_timeout_from_runtime` 从 active snapshot 取得 `DnsCore` handle，确保 service task 与请求入口使用同一 `RuntimeRevision`。正式 `run` 路径由 `Application` 创建 `Arc<RuntimeCoordinator>`，再通过 `DnsService::with_default_timeout_from_coordinator` 共享 coordinator；service 启动时固定一份 listener/runtime 句柄给 transport task，但显式 `reload_prepared` 会按 BindPlan 选择重绑或复用 listener，在 revision CAS 激活后注册新 task 并取消旧 scoped token；新 core 通过 `instrument_core` 继续使用 `DnsService` 持有的 stats/detail/telemetry sink，进程级 `StorageRuntime` 和 `TelemetryWriter` 不随 Runtime revision 重建。resource refresh task 也按 active runtime 的 worker ID 集合使用独立 scoped token，在 reload 时取消旧集合并注册新集合。`PreparedRuntime` 在候选构造阶段装配生产 `ReqwestResourceFetcher`，并由 `ActiveRuntime` 持有其 shared adapter；该 adapter 不进入请求 snapshot。对三类 `auto_update=true` 资源，service 会把按 due/backoff 执行的 refresh worker 注册进同一 `Supervisor`，成功候选在当前 ActiveRuntime 内经 Policy 版本 CAS 和 Runtime 元数据 CAS 生效；资源内容刷新不改变 Runtime revision 或 listener，配置变化才创建候选 Runtime。`RuntimeCoordinator` current-target cell 让旧 Runtime 的 optimistic refresh/late sink 路由到最新 cache/finalizer，并由统一 owner 在 shutdown 时回收；进程持有的 database/logs/webui/resolve-log 配置不能原位替换，Application 文件 reload 会在 async prepare 前返回 `RestartRequired`，service 入口在激活前保留同一防线并继续使用旧 Runtime。正常停机已按 transport/resource task、request drain、cache finalizer、Storage、Telemetry 的顺序完成，故障和 deadline 的完整分项报告仍待完成。
...
`Supervisor::spawn` 继续接收一次性 `TaskFuture`，用于已经由上层持有重建逻辑的 task。需要 supervisor 自己重建的 task 使用 `spawn_with_factory`：factory 每次尝试生成一个新 future，只有 `TaskError::Transient` 且未收到 shutdown cancellation 时才按 `RestartPolicy::Transient { max_restarts }` 有界重试，并在重试间使用可取消的指数退避。需要在 reload 时单独停止的 task 使用 `spawn_scoped`，其 cancellation 仍由 supervisor 持有，并在全局 shutdown 时统一回收；需要同时支持两者的 task 使用 `spawn_scoped_with_factory`。Supervisor 同时记录 Tokio `JoinSet` task ID 到完整 FluxDNS `TaskSpec` 的映射，`JoinError` 的 panic/abort 结果按真实 task 归因并保留 component/fault level，不使用注册表顺序猜测兄弟 task。最终 `TaskCompletion` 携带 `restart_count`，达到上限的瞬时失败可由 `restart_exhausted()` 明确识别；`ShutdownReport` 聚合已发生的重试次数。当前 transport listener task 使用 `FatalEndpoint` 与三次有界瞬时重试；UDP `recv_from` 与 TCP/DoH `accept` 的无流量 `Timeout` 只推进下一轮 deadline，不退出 task 或消耗重试。TCP/DoH listener 的内部 `JoinSet` 各限制为 1,024 个 active session，达到上限时暂停 accept，session 完成或 cancellation 后再继续。resource task 保持 `RestartPolicy::Never` 并在自身循环内 backoff；resource refresh 的启动、成功、失败和 worker 缺失会沿 telemetry health sink 发布低基数状态；显式 service reload 已使用 revision 化 task ID 注册新 listener，并取消旧 token；`DnsService` 已在等待 Ctrl-C 时观察 task completion，并按当前 revision 的 `BindEntry.owner` 聚合 endpoint：单个 endpoint 重试耗尽且同一逻辑 listener 仍有 sibling 时发布 degraded 并继续服务，最后一个 endpoint 耗尽才升级 `ServiceError::TaskFailure`；旧 revision 的迟到耗尽事件不影响新 Runtime。Fatal/FatalEndpoint 最终升级和 panic 仍会在返回原始错误前按同一 grace period 关闭其余 task、Runtime、Storage 和 Telemetry。UDP/TCP/DoH adapter 均通过 `Arc` 持有已绑定句柄，瞬时重试复用该句柄；v1 不再新增会与 Runtime listener ownership 竞争的进程内自动 rebind，只继续补齐 endpoint 故障矩阵。
...
- [x] `DnsService` 观察 Supervisor task completion，区分 Degraded、FatalEndpoint/Fatal、重试耗尽和 panic；单 endpoint 耗尽后保留同组 sibling，逻辑 listener 全部不可用才退出；
- [x] 运行期 fatal/panic 返回原始 task error 前执行有界 shutdown，统一 drain 当前/历史 Runtime 并关闭 Storage/Telemetry；
...
- [x] 验证 transport task 初次执行加三次 transient retry 后才标记耗尽并升级 `FatalEndpoint`；
- [x] 按当前 revision 和逻辑 listener owner 聚合 endpoint 故障，忽略旧 revision 迟到事件；
- [x] 将 UDP/TCP/DoH listener 无流量 `Timeout` 作为正常 deadline 轮询，不消耗 endpoint 重试预算；
- [x] 将 TCP/DoH active session `JoinSet` 固定限制为 1,024，达到上限时暂停 accept；
...
- [x] `DnsService::reload_prepared` 使用同一组进程级 stats/detail/telemetry sink 包装新 core，并验证 stats worker 跨 Runtime revision 连续累计；
- [x] Listener health 在首启、endpoint 降级、成功 reload 和 shutdown 时发布 `Healthy`、`Degraded`、`Healthy` 和 `Stopping`；
...
- [x] 完成正常 drain/shutdown 的 request drain、cache finalizer、Storage checkpoint/flush 和 Telemetry final flush；
- [x] 验证 Supervisor 不合作 task deadline abort 和 Service 活动请求 drain timeout；
- [ ] 完成 shutdown 故障与超时的完整分项报告；
- [ ] 完成并发、故障和时间控制测试。

阶段证据：`runtime::prepared::tests` 验证 async prepare 的 remote restore/fetch、file snapshot load、持久化和 refresh worker 构造；定向测试额外验证 cache persistence 在 bind 前完成 recovery/owner 接线并可有序 shutdown；`runtime::coordinator::tests` 当前 16 项通过，覆盖资源刷新代理、stale-active guard、候选 bind/activate、revision CAS、Runtime drain、finalizer owner 清理与停机摘要合并；`app::tests` 当前 11 项通过，新增用例验证直接 Runtime reload 在 prepare 前拒绝进程持有配置变化；`runtime::supervisor::tests` 覆盖有界重试、scoped cancellation、shutdown report 和 panic 归因；`service::tests::` 聚焦筛选当前 43 项通过，其中主 Service 30 项，覆盖 listener reload/reuse、bind plan 不变时同一地址读取新 Policy answer、候选 bind 失败保留旧 UDP 请求路径、resource task、故障升级、统一 drain、live resource publish、跨 transport 一致性、进程级配置变化时返回 `RestartRequired` 且旧 Runtime 不切换，以及 reload 前后的真实 UDP 请求由同一 stats worker 连续累计。阶段 160 验证正常 shutdown 后 UDP 请求统计已写入 SQLite，阶段 161 验证 fatal task 返回原始错误前同样完成 Runtime drain、SQLite 写入和 Telemetry 关闭，阶段 163 验证 transport task 在初次执行和三次 transient retry 后准确标记耗尽并升级 `FatalEndpoint`，阶段 165 验证同组 sibling 和旧 revision 不会被单 endpoint 耗尽误伤，阶段 166 验证 UDP/TCP/DoH 多轮空闲 deadline 后仍可服务且未发生 task restart，阶段 167 验证 stream session accept 上限边界，阶段 169 验证 Listener health 跨成功 reload 恢复并在 shutdown 进入 `Stopping`。阶段 170 完成 `fmt/check/clippy/test` 大阶段验收，后端全量 `539 passed、0 failed`；阶段 172 验证不合作 task 到期 abort、报告计数和 task 清空，阶段 173 验证活动请求使 Service shutdown 准确报告 deadline，guard 释放后完成 drain，阶段 174 验证 cache persistence adapter 停机超时 abort。Storage/Telemetry/Cache finalizer 的跨阶段强制失败与超时组合尚未完成。

当前实现进度：**90%**。已验证 Runtime snapshot 资源摘要、原子资源 metadata publish、service core 构造入口、生产 ResourceFetcher ownership、production async cache recovery/persistence owner、RuntimeCoordinator 级历史/当前 Policy finalizer owner、无活动旧 finalizer owner 的淘汰清理、同一 ActiveRuntime 内的 remote/file refresh worker/CAS publish、候选 registry 的更高版本合并原语、候选 revision CAS 下的 Policy/worker/metadata 合并、配置 watcher 的 service-aware reload、进程持有配置早期拒绝、listener 重建/复用、Policy answer 切换及 bind 失败回滚、进程级 shared-service 复用、resource worker 按 ID 增量复用/移除、transport listener task 的空闲 deadline 轮询、stream session 固定上限、scoped 有界瞬时重试、同组 endpoint 可用性聚合和旧 revision 故障隔离、Listener health 跨 Runtime 生命周期、当前与旧 Runtime 的 deadline-aware request drain wait、新激活时无活动旧 draining owner 的惰性清理、当前/历史 Runtime 的统一 begin_drain、Supervisor deadline abort、Service request drain timeout、正常停机最终 flush，以及运行期 fatal/panic 返回前的有界收尾。资源内容刷新固定使用同 Runtime per-resource CAS；具体 adapter 故障矩阵和 shutdown 跨阶段强制失败/超时分项报告仍未接线。
