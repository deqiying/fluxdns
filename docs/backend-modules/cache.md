...
- `recover`、`persist`、`maintain_capacity` 与 `shutdown` 复用同一串行 operation lock，锁等待受调用方 deadline 约束；数据库关闭后拒绝继续恢复/写入。
