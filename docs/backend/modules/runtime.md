...
`DnsService::with_default_timeout_from_runtime` 从 active snapshot 取得 `DnsCore` handle，确保 service task 与请求入口使用同一 `RuntimeRevision`。正式 `run` 路径由 `Application` 创建 `Arc<RuntimeCoordinator>`，再通过 `DnsService::with_default_timeout_from_coordinator` 共享 coordinator；service 启动时固定一份 listener/runtime 句柄给 transport task，但显式 `reload_prepared` 会按 BindPlan 选择重绑或复用 listener，在 revision CAS 激活后注册新 task 并取消旧 scoped token。独立 Management listener 也由 Supervisor 持有，但只允许 `webui.users` 随 Runtime reload 替换认证快照；`enable/address/port/public_origin` 变化返回 `RestartRequired`。新 core 继续复用进程级 Storage/Telemetry，resource refresh task 按 active runtime 的 worker ID 使用独立 scoped token。`RuntimeCoordinator` 负责跨 revision cache/finalizer owner；正常停机先撤销 Management session，再处理 transport/resource、request drain、cache finalizer、Storage 和 Telemetry。
