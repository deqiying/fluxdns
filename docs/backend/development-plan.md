...
实施原则：坚持配置驱动，先完成正常运行主线和可用版本；小阶段只做增量改动和定向验证；大阶段结束时才做全量 `cargo fmt`、`cargo check`、`cargo clippy`、`cargo test`。WebUI Management Server 已作为独立 adapter 交付；后续只补充其真实浏览器、双平台发布和故障矩阵验收，不把管理协议重新扩散到 DNS 数据面。DoT/DoQ、主动健康检查和外部 metrics exporter 仍属于后续范围。
