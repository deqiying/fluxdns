...
| E2E-09 指标/进程 | 真实 10 身份在线；服务页与进程页读取同一 RSS 与 CPU；固定窗口、warmup、gap、精确 QPS/RPM、在线容量用例通过 | Linux procfs 与真实 OS 采样失败未实机验证 |
...
服务状态在 2026-09-26 新增 CPU 卡片（卡片行由 4 张变为 5 张）后只由组件测试覆盖；5 卡片布局尚未重跑上述 4 视口浏览器验收。服务页与进程页的 RSS/CPU 一致性由后端 HTTP 用例核对。
