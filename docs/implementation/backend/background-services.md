...
>
> 2026-09-08 增量核对：BC-11 已核对 schema v9、共同水位、每日 scheduler、manifest 物理回收与生产 owner；其余正文保留原核对范围
...
同日 P2 BC-11 增加 3 项 retention 调度/回收测试并扩展生产 owner 与 migration 回归；7 项 retention 定向、91 项 storage 定向均通过，完整 Cargo suite 为 830 passed、0 failed、3 ignored，全部测试目标编译通过。受控墙钟覆盖 01:00 前后、跳过/重复小时、回拨、时区日期变化、retry gate 和跨重启单日一次；真实 stats SQLite 加 3 个详情日覆盖逻辑水位后 checkpoint/delete、注入删除失败记录 failed/attempts、重试 reclaimed、状态范围/时间，以及独立 cache 文件字节不变。Clippy 未新增告警，`-D warnings` 仍被 BC-11 前已有的 6 项 lint 阻断。未验证 Linux 实机、真实权限/磁盘满、发布二进制跨 DST 的长时间运行、完整 v2 冷启/重启、Bearer HTTP/WS、浏览器或约 10 客户端及 core 2ms 性能。
