...
| 存储大小可读单位 | [formatters](../../../frontend/src/shared/formatters/index.ts)、[DnsSettingsPage](../../../frontend/src/modules/dns-settings/DnsSettingsPage.tsx) | `/dns-settings` 缓存内存上限、T 参考大小与详情文件大小按 1024 进制自适应 `KB`／`MB`／`GB`（新增 `formatBytes`），不再输出原始字节数；编辑表单仍按字节数输入 | 2026-09-24：前端 27 文件 121 项 Vitest 与 `pnpm run typecheck`、生产构建通过；用例断言 `512`→`512 B`、`67_108_864`→`64 MB`、`805306368`→`768 MB`、`195454566`→`186.4 MB`、`1_073_741_824`→`1 GB`、`1_073_689_396`→`1 GB`（四舍五入进位）与非法/越界回落 `—`，`App.test.tsx` 断言 DNS 页三处可读文案 | 进程指标 RSS 仍固定 `MiB`；编辑表单未改为「数值 + 单位」；未在真实浏览器复验该页实现视觉 |
