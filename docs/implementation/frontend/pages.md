...
>
> 最后核对：2026-09-21（解析记录列顺序、身份列内容与来源标签分类定向核对，基于 `d7296fd` 之后的本次工作树；其余正文沿用原核对范围）
>
...
解析记录列表按时间、请求、结果、路由、身份排列。身份列以客户端名称为主文本（当前目录已无对应名称时回退到历史匹配 ID），以客户端 IP 和历史匹配来源为辅；原始请求 ID 只在详情中展示。来源标签按缓存状态区分：命中缓存（条目未过 TTL）、乐观缓存（条目已过期但按乐观缓存返回）、缓存过期（条目已过期且不可乐观返回，本次回源）、请求上游（没有条目或缓存未启用回源），`hosts` 保留原标签。
