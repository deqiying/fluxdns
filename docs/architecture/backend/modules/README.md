...
模块设计保留职责、不变量与取舍，完整字段和运行链路继续由 configuration/implementation 维护，不机械派生 12 份实现文档。已验收 D1-D9 保留配置语义与异步主链；环境、组合及长期负载的已知边界见[验证范围与收口](../../../implementation/backend/background-services.md#验证范围与收口)，不以专项结束或文档对齐替代运行证据。
