...
- 共享 fake 与各 adapter 本地用例覆盖相关契约；TLS/proxy/取消/资源上限的实际证据见[DNS 管线](../../../implementation/backend/dns-pipeline.md#契约验证补充)，未验收范围见[验证范围与收口](../../../implementation/backend/background-services.md#验证范围与收口)，不宣称已通过统一 conformance 门禁。
