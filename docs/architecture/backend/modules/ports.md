...
`StatsRecorder` 是 resolution dispatcher 的内部消费端，不再由 DNS 请求任务直接调用。`ResolveEvent` 只保留为详情 projector 使用的持久化 DTO，不是另一个可发布 port。详情 projector 与 SQLite writer 各自使用有界 channel；详情队列满只能影响详情，不能反向阻塞 stats 或 cache commit。统一 ingress 自身溢出则属于整条 resolution event gap，必须显式计数。

`DetailShardStore` 为 v2 历史提供跨日领域读口。authenticated queries 允许 canonical qname、原始身份、历史匹配、当前名称、upstream provenance 和有界 Answer，不返回 wire、request digest、route 文本或原始数据库 row ID。HTTP handler 不持有 SQLx pool；分片 adapter 负责 opaque ID、绑定 SQL、读 lease 与共同水位。旧 ManagementStorageRead 和 legacy 投影已删除。
...
生产 Runtime 不执行 contract suite，也没有统一的 conformance 注册门禁。测试夹具存在不等于每个 adapter 的所有组合都已验收；已覆盖用例见[Adapter 支持矩阵](../../../implementation/backend/dns-pipeline.md#adapter-支持矩阵)，未验收范围见[验证范围与收口](../../../implementation/backend/background-services.md#验证范围与收口)。
