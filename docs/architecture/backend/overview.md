...
模块中的“契约验证要求”是应证明的行为，不是测试通过记录。跨 adapter conformance、真实磁盘故障、Unix 进程信号及目标负载压力的证据边界见[验证范围与收口](../../implementation/backend/background-services.md#验证范围与收口)；验证专项结束不改变已验收 D1-D9 的设计选择，也不以本机 loopback、fake 或静态接线替代目标环境验收。
