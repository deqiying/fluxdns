...
use super::contract::{
    ClientMessage, ResyncReason, Revision, ServerMessage, WS_CONNECTION_CAPACITY,
...
impl EventHub {
    pub(super) fn new(
        ...
        let epoch = Revision::try_from(epoch)?;
                        ...
                        Message::Text(text) => {
                            ...
                            match message {
                                ClientMessage::SubscribeMetrics { subscription_id } => {
                                    if !metrics_subscriptions.contains(subscription_id.as_str())
                                        && metrics_subscriptions.len() >= WS_SUBSCRIPTIONS_PER_CONNECTION
                                    {
                                        outbound.close(1008, "subscription limit exceeded");
                                        break;
                                    }
                                    metrics_subscriptions.insert(subscription_id.as_str().to_owned());
                                ...
                                ClientMessage::SubscribeQueries { subscription_id, .. } => {
                                    if !outbound.json(&ServerMessage::ResyncRequired {
                                        subscription_id,
                                        reason: ResyncReason::ObservationGap,
                                    }) {
                                        break;
                                    }
                                }
...
mod tests {
    ...
    async fn receive_server_message(
        ...
        let ClientFrame::Text(text) = message else {
            panic!("expected text server message");
        };
